...
/// QRVMC execution message structure.
#[derive(Debug)]
...
/// QRVMC transaction context structure.
pub type ExecutionTxContext = ffi::qrvmc_tx_context;

/// QRVMC context structure. Exposes the QRVMC host functions, message data, and transaction context
/// to the executing VM.
pub struct ExecutionContext<'a> {
    host: &'a ffi::qrvmc_host_interface,
    context: *mut ffi::qrvmc_host_context,
...
impl ExecutionResult {
    ...
    pub fn failure() -> Self {
        ExecutionResult::new(StatusCode::QRVMC_FAILURE, 0, 0, None)
    }

    /// Create a revert result.
    pub fn revert(_gas_left: i64, _output: Option<&[u8]>) -> Self {
        ExecutionResult::new(StatusCode::QRVMC_REVERT, _gas_left, 0, _output)
    }

    /// Create a successful result.
    pub fn success(_gas_left: i64, _gas_refund: i64, _output: Option<&[u8]>) -> Self {
        ExecutionResult::new(StatusCode::QRVMC_SUCCESS, _gas_left, _gas_refund, _output)
    }
...
impl<'a> ExecutionContext<'a> {
    ...
    pub fn call(&mut self, message: &ExecutionMessage) -> ExecutionResult {
        ...
        // qrvmc_message doesn't have a release() method we could abstract it with.
        let message = ffi::qrvmc_message {
            kind: message.kind(),
            flags: message.flags(),
            depth: message.depth(),
            gas: message.gas(),
        ...
        unsafe {
            ...
            (*self.host).call.unwrap()(self.context, &message as *const ffi::qrvmc_message).into()
        }
    }
...
impl From<ffi::qrvmc_result> for ExecutionResult {
    fn from(result: ffi::qrvmc_result) -> Self {
        let ret = Self {
        ...
        if result.release.is_some() {
            unsafe {
                result.release.unwrap()(&result as *const ffi::qrvmc_result);
            }
        }
...
/// Returns a pointer to a heap-allocated qrvmc_result.
impl From<ExecutionResult> for *const ffi::qrvmc_result {
    fn from(value: ExecutionResult) -> Self {
        let mut result: ffi::qrvmc_result = value.into();
...
extern "C" fn release_heap_result(result: *const ffi::qrvmc_result) {
    unsafe {
        let tmp = Box::from_raw(result as *mut ffi::qrvmc_result);
        deallocate_output_data(tmp.output_data, tmp.output_size);
    }
}

/// Returns a pointer to a stack-allocated qrvmc_result.
impl From<ExecutionResult> for ffi::qrvmc_result {
    fn from(value: ExecutionResult) -> Self {
...
/// Callback to pass across FFI, de-allocating the optional output_data.
extern "C" fn release_stack_result(result: *const ffi::qrvmc_result) {
    unsafe {
        let tmp = *result;
        deallocate_output_data(tmp.output_data, tmp.output_size);
    }
...
impl From<&ffi::qrvmc_message> for ExecutionMessage {
    fn from(message: &ffi::qrvmc_message) -> Self {
        ExecutionMessage {
            kind: message.kind,
            flags: message.flags,
            depth: message.depth,
...
mod tests {
    ...
    #[test]
    fn result_new() {
        let r = ExecutionResult::new(StatusCode::QRVMC_FAILURE, 420, 21, None);

        assert_eq!(r.status_code(), StatusCode::QRVMC_FAILURE);
    ...
    extern "C" fn test_result_dispose(result: *const ffi::qrvmc_result) {
        unsafe {
            if !result.is_null() {
                let owned = *result;
                Vec::from_raw_parts(
    ...
    #[test]
    fn result_from_ffi() {
        let f = ffi::qrvmc_result {
            status_code: StatusCode::QRVMC_SUCCESS,
            gas_left: 1337,
            gas_refund: 21,
        ...
        assert_eq!(r.status_code(), StatusCode::QRVMC_SUCCESS);
    ...
    fn result_into_heap_ffi() {
        ...
        let f: *const ffi::qrvmc_result = r.into();
        assert!(!f.is_null());
        unsafe {
            assert_eq!((*f).status_code, StatusCode::QRVMC_FAILURE);
    ...
    fn message_new_with_input() {
        ...
        assert_eq!(ret.kind(), MessageKind::QRVMC_CALL);
    ...
    fn message_from_ffi() {
        ...
        let msg = ffi::qrvmc_message {
            kind: MessageKind::QRVMC_CALL,
            flags: 44,
            depth: 66,
            gas: 4466,
            recipient,
    ...
    unsafe extern "C" fn execute_call(
        ...
        ffi::qrvmc_result {
            status_code: if success {
                ...
                StatusCode::QRVMC_INTERNAL_ERROR
            },
            gas_left: 2,
            gas_refund: 0,
    ...
    fn get_dummy_host_interface() -> ffi::qrvmc_host_interface {
        ffi::qrvmc_host_interface {
            account_exists: None,
            get_storage: None,
            set_storage: None,
            get_balance: None,
    ...
    fn test_call_empty_data() {
        ...
        assert_eq!(b.status_code(), StatusCode::QRVMC_SUCCESS);
